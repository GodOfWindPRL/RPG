/**
 * Đồng bộ với `frontend/src/core/forestObstacles.ts` (seed, count, margin, bán kính).
 */
import { WORLD_MAP_HALF_SIZE } from './worldBounds.js';
...
export function computeForestScatterXZ(): { x: number; z: number }[] {
  ...
  const span = WORLD_MAP_HALF_SIZE - FOREST_EDGE_MARGIN;
...
let cached: { x: number; z: number; r: number }[] | null = null;

export function getForestObstacleCircles(): { x: number; z: number; r: number }[] {
  if (!cached) {
    cached = computeForestScatterXZ().map((p) => ({ x: p.x, z: p.z, r: FOREST_PROP_RADIUS }));
  }
  return cached;
}
